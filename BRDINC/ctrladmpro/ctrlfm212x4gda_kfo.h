...
typedef struct  _FM212X4GDA_KFO_KGALL {
	U32		isKg;				// включение/выключение кодограммы: 1-вкл., 0-выкл.
	U32		kgDuration;			// длительность цикла кодограммы
	U32		loopCnt;			// количество повторов кодограммы
	U32		infinite;			// бесконечный цикл. 0 - однократный
	U32		aStbSet[6];			// установка стробов 0-5. Если 0 - строб выключен
	U32		aStbClr[6];			// сброс стробов 0-5. Если 0 - строб выключен
	U32		aStbInv[6];			// инверсия стробов 0-5. Если 0 - нет инверсии
} FM212X4GDASRV_KFO_KGALL,*PFM212X4GDASRV_KFO_KGALL;

typedef struct  _FM212X4GDA_KFO_KGONE {
	U32		stbIdx;				// номер программируемого строба: 0-5
	U32		stbSet;				// установка строба. Если 0 - строб выключен
	U32		stbClr;				// сброс строба. Если 0 - строб выключен
	U32		stbInv;				// инверсия строба. Если 0 - нет инверсии
